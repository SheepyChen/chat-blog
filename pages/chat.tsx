...
const Index: React.FC = () => {
  ...
  useEffect((): any => {
    ...
    socketClient.on("message", (message: IMsg) => {
      chat.push(message);
      setChat([...chat]);
    });
          ...
          {chat.length ? (
            chat.map((chat, i) => (
              <div key={"msg_" + i} className="mt-1">
                <span
                  className={
                    chat.user === user ? "text-blue-500" : "text-black"
                  }
                >
          ...
          <div className="flex flex-row flex-1 h-full divide-gray-200 divide-x">
            <div className="pr-2 flex-1">
              <input
                ...
                className="w-full h-full rounded shadow border-gray-700 border px-2 text-black"
                disabled={!connected}
                onChange={(e) => {
